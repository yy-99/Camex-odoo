...
class InstagramMessage(models.Model):
    _name = 'instagram.message'
    _description = 'Instagram Messages'
    _order = 'id desc'
    ...
    def _send_message(self, with_commit=False):
        ...
        # init api
        message_to_api = {}
        for account, messages in groupby(self, lambda msg: msg.meta_social_account_id):
            if not account:
        ...
        for whatsapp_message in self:
            ...
            try:
                ...
                # no template
                    ...
                    attachment_vals = whatsapp_message._prepare_attachment_vals(whatsapp_message.mail_message_id.attachment_ids[0], meta_social_account_id=whatsapp_message.meta_social_account_id)
    ...
    def _post_message_in_active_channel(self):
        ...
        if not self.wa_template_id:
            return
        channel = self.meta_social_account_id._find_active_channel(self.mobile_number_formatted)
        if not channel:
            return
    ...
    @api.model
    def _prepare_attachment_vals(self, attachment, meta_social_account_id):
        ...
        wa_api = WhatsAppApi(meta_social_account_id)
